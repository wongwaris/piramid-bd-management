...
import { normalizeEmails } from './utils.js';
...
export class EmailQueueWorker {
  ...
  async enqueue(input) {
    const toEmails = normalizeEmails(input.to);
    const ccEmails = normalizeEmails(input.cc);
    const bccEmails = normalizeEmails(input.bcc);
    const rows = await this.sql`
      ...
      values (
        ...
        ${json(toEmails)}::jsonb, ${json(ccEmails)}::jsonb, ${json(bccEmails)}::jsonb,
